...
resource "vercel_project_environment_variable" "presence_hub_url" {
  count      = var.manage_vercel ? 1 : 0
  project_id = vercel_project.web[0].id
  key        = "NEXT_PUBLIC_PRESENCE_HUB_URL"
  value      = "https://${azurerm_container_app.identity_presence.latest_revision_fqdn}/presence/hub"
  target     = ["production", "preview"]
  sensitive  = false
}

resource "vercel_project_environment_variable" "events_hub_url" {
  count      = var.manage_vercel ? 1 : 0
  project_id = vercel_project.web[0].id
  key        = "NEXT_PUBLIC_EVENTS_HUB_URL"
