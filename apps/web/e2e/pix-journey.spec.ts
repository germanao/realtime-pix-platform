...
test("simple mode sends one PIX, preserves context in Expert mode, and grants welcome money once", async ({
  ...
  await page.getByRole("button", { name: "Aurora Ledger [BOT] Always available" }).click();
  ...
  await expect(page.getByText(/^(Sent to Aurora Ledger \[BOT\]|PIX sent)$/)).toBeVisible({ timeout: 15_000 });
