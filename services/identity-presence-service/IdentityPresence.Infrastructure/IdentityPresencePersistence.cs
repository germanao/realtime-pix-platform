    ...
    private async Task<PresenceUserEntity> UpsertAnonymousUserAsync(string normalizedClientId, CancellationToken cancellationToken)
    {
        ...
        if (user is null)
        {
            user = new PresenceUserEntity
            {
                ...
                DisplayName = AnonymousDisplayNames.Create(normalizedClientId),
                IsBot = false,
                LastSeenAt = now
            };
        ...
        {
            user.DisplayName = AnonymousDisplayNames.Create(normalizedClientId);
            user.IsBot = false;
            user.LastSeenAt = now;
        }
...
public sealed class IdentityPresenceDbContextFactory : IDesignTimeDbContextFactory<IdentityPresenceDbContext>
{
    ...
    {
        var connectionString = Environment.GetEnvironmentVariable("IDENTITY_PRESENCE_MIGRATIONS_CONNECTION")
            ?? throw new InvalidOperationException(
                "Set IDENTITY_PRESENCE_MIGRATIONS_CONNECTION before using EF Core design-time tooling.");
